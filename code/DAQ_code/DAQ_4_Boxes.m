...
%% Creating DAQ session tracking running voltage for specified period of time
    ...
    s.DurationInSeconds=10; % This is the duration of the recording in seconds.
...
%% Applying a Butterworth band-pass filter to the raw data
    ...
    data_h_4 = filtfilt(f_high,data(:,4)); % Band-passing the raw data on Box 

%% Baseline correction and tranformation to absolute values

    Mh_4 = mean(data_h_4); % Mean of filtered data
    data_h_4 = data_h_4 - Mh_4; % Approaching the baseline to an average of 0
    absh_4 = abs(data_h_4); % Absolute value transformation

%% Setting the threshold

    sd_4 = std(data_h_4); % SD of the band-passed data

    if sd_4 * nSD > Ttv % Conditional for setting the value of the threshold
        threshold = Ttv; % Top threshold value for datasets with higher SD
    else
        threshold = sd_4 * nSD; % Threshold set to n-fold SD (datasets with lower SD)
    end

%% Unconditional detection of local maxima

    [pksh_4,loch_4] = findpeaks(absh_4); % Prominence of local maxima stored in "pksh", their time indices are stored in "loch"

%% Conditional detection of local maxima for identification of HTR events

    [pksh2_4,loch2_4,wh2_4] = findpeaks(pksh_4,loch_4,'MinPeakHeight',threshold,'MinPeakDist',mpd,'MaxPeakWidth',Mpw); % Prominence, time position and width of local maxima --applied over the previous local maxima detection-- stored if the detection parameters are met. In other words, if a HTR event is detected.
    % "pksh2_4" contains the prominence (V) of each HTR event detected
    % "loch2_4" contains the time (ms) of occurrence of each HTR event detected.
    % This are the timestamps used for matching visually identified events.
    % "wh2_4" contains the width (ms) of each HTR event detected
    figure();
    findpeaks(pksh_4,loch_4,'MinPeakHeight',threshold,'MinPeakDist',mpd,'MaxPeakWidth',Mpw);
    txt=['Number of peaks is ',num2str(size(loch2_4,1))]; % Pastes in words on the graph the number of peaks (HTR events) detected by the code
    text(1,0.3,txt);
    xlabel('Time (ms)');
    ylabel('Signal (|V|)');
    title('Box 4');
    t_4=milliseconds(loch2_4);
    t_4.Format='mm:ss.SSS';
    X_4='The time points of the head twitches for Box 4 are:';
    disp(X_4);
    disp(t_4);
    save data.mat data
    % To load the data on a different computer, type load('data.mat').
